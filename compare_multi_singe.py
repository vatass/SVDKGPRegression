...
REGION_ROIS = OrderedDict({
    0: ("Limbic system", [
        "100","101","116","117","138","139","166","167","170","171"
    ]),
    1: ("Parietal lobe", [
        "85","86","106","107","148","149","168","169","176","177",
        "194","195","198","199",
    ]),
    ...
    3: ("Temporal lobe", [
        "87","88","122","123","132","133","154","155",
        "180","181","184","185","200","201","202","203","206","207"
    ]),
    4: ("Occipital lobe", [
        "83","84","108","109","114","115","128","129",
        "134","135","144","145","156","157","160","161","196","197",
    ]),
    5: ("Frontal lobe", [
        "81","82","102","103","104","105","112","113",
...
# =========================================================
# MULTITASK PARSER (ALL LAMBDAS)
# =========================================================

def parse_multitask_results_all_lambdas(txt_path):
    ...
    with open(txt_path, "r") as f:
        for line in f:
            ...
            if line.startswith("Lambda penalty value:"):
                current_lambda = float(line.split(":")[1].strip())
                results.setdefault(current_lambda, {})

            if line.startswith("Mode:"):
                current_mode = int(line.split(":")[1].strip())
                current_block = {}

            if "Mean Test MSE" in line:
                current_block["MSE"] = float(line.split(":")[-1])

            if "Mean Test MAE" in line:
                current_block["MAE"] = float(line.split(":")[-1])

            if "Mean Test Coverage%" in line:
                current_block["Coverage"] = float(line.split(":")[-1])

            if "Mean Test Interval Width" in line:
                current_block["Width"] = float(line.split(":")[-1])

                # End of block → store
                results[current_lambda][current_mode] = current_block.copy()
...
def main():
    ...
    multitask_all = parse_multitask_results_all_lambdas(TXT_PATH)

    print("\n=========== REGION COMPARISON (FOLD 0 ONLY) ===========\n")

    for lambda_value in sorted(multitask_all.keys()):

        print("\n" + "=" * 70)
        print(f"Lambda penalty value: {lambda_value}")
        print("=" * 70)

        multitask = multitask_all[lambda_value]
